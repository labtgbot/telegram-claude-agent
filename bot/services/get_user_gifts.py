...
GET_USER_GIFTS_MIN_LIMIT = 1
GET_USER_GIFTS_MAX_LIMIT = 100


class GetUserGiftsError(Exception):
    """Raised when ``getUserGifts`` validation or call fails."""
...
def normalize_get_user_gifts_options(
    *,
    ...
    """Validate optional ``getUserGifts`` filters."""
    options = {
    ...
    for key, value in options.items():
        if not isinstance(value, bool):
            raise GetUserGiftsError(f"{key} must be a boolean.")
    ...
    if offset is not None:
        offset = offset.strip()
        if not offset:
            raise GetUserGiftsError("offset must be non-empty.")
        payload["offset"] = offset
    if limit is not None:
        if not isinstance(limit, int) or isinstance(limit, bool):
            raise GetUserGiftsError("limit must be an integer.")
        if not (GET_USER_GIFTS_MIN_LIMIT <= limit <= GET_USER_GIFTS_MAX_LIMIT):
            raise GetUserGiftsError("limit must be between 1 and 100.")
        payload["limit"] = limit
    return payload


async def perform_get_user_gifts(
    bot: Any,
    *,
    user_id: int,
    exclude_unsaved: bool = False,
    ...
    """Fetch gifts owned by a Telegram user."""
    if not isinstance(user_id, int) or isinstance(user_id, bool) or user_id <= 0:
        raise GetUserGiftsError("user_id must be a positive integer.")

    payload = {
        "user_id": user_id,
        **normalize_get_user_gifts_options(
            exclude_unsaved=exclude_unsaved,
            exclude_saved=exclude_saved,
            exclude_unlimited=exclude_unlimited,
            exclude_limited=exclude_limited,
    ...
    url = _build_api_url(bot, "getUserGifts")
    ...
    try:
        ...
        logger.warning(
            "get_user_gifts_failed",
            user_id=user_id,
            error_type=type(exc).__name__,
            error=str(exc),
        )
        raise GetUserGiftsError(f"getUserGifts request failed: {exc}") from exc
    ...
    if not data.get("ok"):
        ...
        raise GetUserGiftsError(description, error_code=error_code)
    ...
    if not isinstance(result, dict) or not isinstance(result.get("gifts"), list):
        ...
        raise GetUserGiftsError(
            "Telegram returned an unexpected user gifts result."
        )
...
def format_user_gifts(gifts: dict[str, Any], *, user_id: int) -> str:
    ...
    lines = [
        "<b>User gifts</b>",
        f"User: <code>{escape(str(user_id))}</code>",
