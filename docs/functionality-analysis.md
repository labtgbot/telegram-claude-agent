...
| `uploadStickerFile` | `bot/services/upload_sticker_file.py`, `/uploadstickerfile` в `bot/handlers/commands.py` | Admin-flow загрузки локального sticker-файла для последующего lifecycle sticker sets; команда закрыта строгим `TELEGRAM_ADMIN_CHAT_IDS` без fallback и deny-by-default при пустом списке, принимает `user_id`, `sticker_format` (`static`, `animated`, `video`) и путь к файлу на bot host; используется изолированный raw multipart Bot API helper для совместимости с pinned `aiogram==3.3.0`, результат парсится в aiogram `File` и возвращает `file_id`; команда не вызывает `free-claude-code`, не требует специальных update types, меняет только Telegram file storage для выбранного пользователя, а ошибки локальной валидации, транспорта, Telegram API validation или rate limit возвращаются оператору. |
| `createNewStickerSet` | `bot/services/create_new_sticker_set.py`, `/createnewstickerset` в `bot/handlers/commands.py` | Admin-flow создания sticker/custom emoji set с одним заранее загруженным `sticker_file_id` и списком emoji; команда закрыта строгим `TELEGRAM_ADMIN_CHAT_IDS` без fallback и deny-by-default при пустом списке, принимает `user_id`, уникальное имя set, title, `sticker_type` (`regular`, `mask`, `custom_emoji`), `sticker_format` (`static`, `animated`, `video`), `file_id` и emoji list; используется изолированный raw Bot API helper для совместимости с pinned `aiogram==3.3.0`; метод не требует специальных update types, но создает Telegram state для выбранного пользователя, требует соблюдения Telegram naming/ownership rules, rollback выполняется через lifecycle-инструменты sticker set, а ошибки локальной валидации, транспорта, Telegram API validation или rate limit возвращаются оператору. |
...
| `setStickerSetTitle` | `bot/services/set_sticker_set_title.py`, `/setstickersettitle` в `bot/handlers/commands.py` | Admin-flow изменения title bot-created sticker set по `sticker_set_name` и новому `title`; команда закрыта строгим `TELEGRAM_ADMIN_CHAT_IDS` без fallback и deny-by-default при пустом списке, валидирует непустой `name`, непустой `title` и лимит Telegram 64 символа до обращения к Telegram, использует изолированный raw Bot API helper для совместимости с pinned `aiogram==3.3.0`, не требует специальных update types, так как сценарий запускается обычной admin-командой, не вызывает `free-claude-code`, работает только со sticker sets, созданными ботом, rollback выполняется повторным вызовом с прежним title из `/getstickerset` или operational notes, а ошибки транспорта, Telegram API validation или rate limit возвращаются оператору. |
...
| `sendInvoice` | `bot/services/send_invoice.py`, `/sendinvoice` в `bot/handlers/commands.py` | Узкий admin-flow отправки Telegram Stars test invoice в текущий чат по `star_count`, `payload`, `title` и `description`; используется изолированный raw Bot API helper, prices JSON-сериализуются, `provider_token` передается пустым, `currency=XTR`, команда закрыта строгим `TELEGRAM_ADMIN_CHAT_IDS` без fallback и deny-by-default при пустом списке, а полноценный paid сценарий требует отдельного billing domain, подписанного payload, audit log и обработки `pre_checkout_query`/`successful_payment` update types. |
| `createInvoiceLink` | `bot/services/create_invoice_link.py`, `/createinvoicelink` в `bot/handlers/commands.py` | Узкий admin-flow создания Telegram Stars invoice link по `star_count`, `payload`, `title` и `description`; используется изолированный raw Bot API helper для совместимости с pinned `aiogram==3.3.0`, prices JSON-сериализуются, `provider_token` передается пустым, `currency=XTR`, команда закрыта строгим `TELEGRAM_ADMIN_CHAT_IDS` без fallback и deny-by-default при пустом списке, возвращенная ссылка отправляется только оператору, а production billing требует подписанного/idempotent payload, audit log и обработки `pre_checkout_query`/`successful_payment` update types. |
| `answerShippingQuery` | `bot/services/answer_shipping_query.py` | Явно оставлен blocked-by-product: текущий продукт не продает physical goods, не запрашивает shipping address и не имеет shipping-options catalog, поэтому `shipping_query` handler не регистрируется и Telegram `answerShippingQuery` не вызывается; сервис фиксирует решение, пишет только структурную метаинформацию и всегда поднимает dedicated blocked-by-product error для positive и negative answer paths. Разблокировка требует отдельного physical-goods checkout design, fulfillment/rollback policy, signed payload/idempotency, `shipping_query` allowed update и unit/security tests. |
...
| `setPassportDataErrors` | `bot/services/set_passport_data_errors.py`, `/setpassporterrors` в `bot/handlers/commands.py` | Узкий admin-flow возврата Telegram Passport validation errors пользователю по `user_id` и non-empty JSON array `PassportElementError`; используется изолированный raw Bot API helper, так как pinned `aiogram==3.3.0` не имеет typed wrapper для этого метода, команда закрыта строгим `TELEGRAM_ADMIN_CHAT_IDS` без fallback и deny-by-default при пустом списке, не требует chat administrator rights и не вызывает `free-claude-code`; product-flow должен отдельно получать `passport_data` message updates, расшифровывать/валидировать данные и формировать error hashes, а structured logs не пишут содержимое Passport errors. |
...
| `refundStarPayment` | `bot/services/refund_star_payment.py`, `/refundstars` в `bot/handlers/commands.py` | Destructive admin-flow возврата Telegram Stars payment пользователю по `user_id` и `telegram_payment_charge_id`; используется изолированный raw Bot API helper, так как pinned `aiogram==3.3.0` не имеет typed wrapper для этого Bot API 10.0 метода; команда доступна только в `TELEGRAM_ADMIN_CHAT_IDS`, требует явный `confirm`, не падает обратно на `TELEGRAM_ALLOWED_CHAT_IDS`, отделена от invoice/pre-checkout handlers и `free-claude-code`, а in-process идемпотентность не вызывает Telegram повторно для той же пары user/charge в рамках процесса; structured logs содержат user id, charge id и форму ошибки для audit. |
| `editUserStarSubscription` | `bot/services/edit_user_star_subscription.py`, `/edituserstarsubscription` в `bot/handlers/commands.py` | Destructive admin-flow отмены будущего renewal или реактивации Telegram Stars subscription по `user_id`, `telegram_payment_charge_id` и target state `active|canceled`; используется изолированный raw Bot API helper, так как pinned `aiogram==3.3.0` не имеет typed wrapper для этого Bot API 10.0 метода; команда доступна только в `TELEGRAM_ADMIN_CHAT_IDS`, требует явный `confirm`, не падает обратно на `TELEGRAM_ALLOWED_CHAT_IDS`, отделена от invoice/pre-checkout handlers и `free-claude-code`, а in-process идемпотентность не вызывает Telegram повторно для той же тройки user/charge/state в рамках процесса; structured logs содержат user id, charge id, target state и форму ошибки для audit. |
...
5. Интерактивность: полноценные `answerInlineQuery` ответы через Claude,
   ...
   Passport validation: `setPassportDataErrors` уже интегрирован как
   admin-only raw helper; полноценный product-flow требует отдельной обработки
   `passport_data` updates и доменной валидации.
...
Команда `/setstickerkeywords` вызывает Telegram Bot API `setStickerKeywords`
через изолированный raw Bot API helper, потому что проект закреплен на
`aiogram==3.3.0`. По официальной документации метод принимает обязательный
`sticker` и необязательный список `keywords`; список может содержать до 20
ключевых слов, а пустой список очищает keywords у sticker. Локальная валидация
отклоняет пустой `sticker` и превышение лимита до обращения к Telegram.

Метод меняет Telegram state и применяется только к stickers из sets, созданных
ботом. Специальные update types не требуются: сценарий запускается обычной
командой из admin-чата. Сценарий выбран как admin operation для
creative/media module и lifecycle sticker sets: оператор сначала находит
`sticker_file_id` через `/getstickerset`, затем задает поисковые keywords для
sticker/custom emoji. Команда не участвует в основном Claude chat flow и не
вызывает `free-claude-code`.

Синтаксис: `/setstickerkeywords <sticker_file_id> <keyword[,keyword...]|->`.

`/setstickerkeywords` закрыт строгим admin allowlist:
...
- при отсутствующих аргументах команда показывает usage и не обращается к
  Telegram;
- `-` передается как пустой список и очищает keywords;
- ошибки локальной валидации, транспорта, Telegram API validation и rate limit
  возвращаются оператору.

Rollback ручной: повторить `/setstickerkeywords` с прежними keywords,
сохраненными в operational notes или полученными через внешние Telegram tools.
Structured logs пишут только количество keywords, без самих значений.

### setStickerSetTitle

Команда `/setstickersettitle` вызывает Telegram Bot API `setStickerSetTitle`
через изолированный raw Bot API helper, потому что проект закреплен на
`aiogram==3.3.0`. По официальной документации метод принимает обязательные
`name` и `title`; `title` должен быть непустым и не длиннее 64 символов.
Локальная валидация отклоняет пустой `name`, пустой `title` и превышение
лимита до обращения к Telegram.

Метод меняет Telegram state и применяется только к sticker sets, созданным
ботом. Специальные update types не требуются: сценарий запускается обычной
командой из admin-чата. Сценарий выбран как admin operation для
creative/media module и lifecycle sticker sets: оператор сначала проверяет
текущий set через `/getstickerset`, затем меняет отображаемый title. Команда
не участвует в основном Claude chat flow и не вызывает `free-claude-code`.

Синтаксис: `/setstickersettitle <sticker_set_name> <title>`.

`/setstickersettitle` закрыт строгим admin allowlist:
...
Rollback ручной: повторить `/setstickersettitle` с прежним title, сохраненным
из `/getstickerset` или operational notes. Structured logs пишут только имя set,
длину title, тип ошибки и Telegram error code.

### setStickerSetThumbnail

Команда `/setstickersetthumbnail` вызывает Telegram Bot API
`setStickerSetThumbnail` через изолированный raw Bot API helper, потому что
проект закреплен на `aiogram==3.3.0`. Метод принимает обязательные `name`,
`user_id` и `format`, а также опциональный `thumbnail`; команда требует
thumbnail `file_id` или `-` для очистки текущего thumbnail. `format` должен быть
`static`, `animated` или `video`. Локальная валидация отклоняет неположительный
`user_id`, пустой `name`, неизвестный `format` и пустой thumbnail до обращения к
Telegram.
...
командой из admin-чата. Сценарий выбран как admin operation для
creative/media module и lifecycle sticker sets: оператор сначала проверяет
текущий set через `/getstickerset`, затем задает или очищает thumbnail. Команда
не участвует в основном Claude chat flow и не вызывает `free-claude-code`.

Синтаксис: `/setstickersetthumbnail <user_id> <sticker_set_name> <format> <thumbnail_file_id|->`.

`/setstickersetthumbnail` закрыт строгим admin allowlist:
...
Rollback ручной: повторить `/setstickersetthumbnail` с прежним thumbnail file id,
сохраненным из `/getstickerset` или operational notes, либо очистить thumbnail
через `-`. Structured logs пишут только имя set, `user_id`, `format`, факт
наличия thumbnail, тип ошибки и Telegram error code.

### setCustomEmojiStickerSetThumbnail

Команда `/setcustomemojithumbnail` вызывает Telegram Bot API
`setCustomEmojiStickerSetThumbnail` через изолированный raw Bot API helper,
потому что проект закреплен на `aiogram==3.3.0`. Метод принимает обязательный
`name` и опциональный `custom_emoji_id`; команда требует custom emoji id или
`-` для очистки текущего thumbnail. Локальная валидация отклоняет пустой `name`
и пустой `custom_emoji_id` до обращения к Telegram.

Метод меняет Telegram state и применяется только к custom emoji sticker sets,
созданным ботом. Специальные update types не требуются: сценарий запускается
обычной командой из admin-чата. Сценарий выбран как admin operation для
creative/media module и lifecycle sticker sets: оператор сначала проверяет
текущий set через `/getstickerset`, затем задает custom emoji из этого set как
thumbnail или очищает thumbnail. Команда не участвует в основном Claude chat
flow и не вызывает `free-claude-code`.

Синтаксис: `/setcustomemojithumbnail <sticker_set_name> <custom_emoji_id|->`.

`/setcustomemojithumbnail` закрыт строгим admin allowlist:
...
Команда `/mystarbalance` получает Telegram `StarAmount` для текущего бота
методом `getMyStarBalance` (Bot API 10.0). Это read-only admin diagnostic для
проверки доступного bot Stars balance перед отдельными invoice, refund, gift и
subscription flows.

Pinned `aiogram==3.3.0` не имеет typed wrapper для этого метода, поэтому
реализация идет через изолированный raw Bot API helper
`bot/services/get_my_star_balance.py`. Helper POST'ит пустой JSON payload на
endpoint `getMyStarBalance` через `httpx`, берет URL через
`bot.session.api.api_url(...)` для поддержки local Bot API server и поднимает
транспортные ошибки или Telegram `ok: false` как `GetMyStarBalanceError`.

Сценарий защищен strict admin allowlist:
`/mystarbalance` доступен только chat id из `TELEGRAM_ADMIN_CHAT_IDS` и не
делает fallback на `TELEGRAM_ALLOWED_CHAT_IDS`; при пустом admin allowlist
команда отключена. Метод не принимает параметров, не требует специальных
update types или chat administrator rights, потому что сценарий запускается
обычной командой из admin-чата.

Security/privacy impact: команда раскрывает финансовый баланс Telegram Stars
бота, поэтому результат показывается только в admin-чате. Structured logs пишут
...
Команда `/refundstars <user_id> <telegram_payment_charge_id> confirm`
возвращает Telegram Stars payment пользователю методом `refundStarPayment`
(Bot API 10.0). Метод принимает обязательные `user_id` и
`telegram_payment_charge_id`; charge id должен приходить из trusted billing
records, `successful_payment` update или audit/reconciliation через
`/startransactions`.

Pinned `aiogram==3.3.0` не имеет typed wrapper для этого метода, поэтому
реализация идет через изолированный raw Bot API helper
`bot/services/refund_star_payment.py`. Helper валидирует положительный integer
`user_id` и непустой charge id, POST'ит JSON payload
`{"user_id": ..., "telegram_payment_charge_id": ...}` на endpoint
`refundStarPayment` через `httpx`, берет URL через
`bot.session.api.api_url(...)` для поддержки local Bot API server и поднимает
validation errors, транспортные ошибки, невалидный JSON, Telegram `ok: false`
и неожиданный result как `RefundStarPaymentError`.

Сценарий намеренно отделен от `/sendinvoice`, `/createinvoicelink`,
`/startransactions`, shipping/pre-checkout callbacks и `free-claude-code`:
оператор сначала сверяет billing audit log и charge id, затем запускает refund
отдельной командой с явным `confirm`. Для самой admin-команды не нужны
специальные update types или chat administrator rights; production billing,
который формирует данные для refund, должен сохранять `pre_checkout_query` и
`successful_payment` audit data отдельно.

Команда доступна только chat id из `TELEGRAM_ADMIN_CHAT_IDS` и не делает
fallback на `TELEGRAM_ALLOWED_CHAT_IDS`; при пустом admin allowlist команда
отключена. Handler хранит in-process idempotency set по паре
`(user_id, telegram_payment_charge_id)`, поэтому повторная подтвержденная
команда в рамках того же процесса не вызывает Telegram второй раз и возвращает
audit-сообщение о ранее записанном refund.

Security/privacy impact: команда меняет финансовое состояние Stars payment,
поэтому она не вызывается автоматически и требует отдельный admin allowlist.
Structured logs содержат user id, charge id и форму ошибки для audit, но не
полный payment payload. Rollback ограничен операционно: убрать chat id из
`TELEGRAM_ADMIN_CHAT_IDS`, удалить handler или отключить billing module вокруг
helper; уже принятый Telegram refund ботом не откатывается.

### editUserStarSubscription

Команда `/edituserstarsubscription <user_id> <telegram_payment_charge_id>
active|canceled confirm` меняет состояние Telegram Stars subscription методом
`editUserStarSubscription` (Bot API 10.0). Метод принимает обязательные
`user_id`, `telegram_payment_charge_id` и `is_canceled`; значение `canceled`
мапится в `is_canceled=true`, а `active` - в `is_canceled=false`. User id и
charge id должны приходить из trusted billing records, subscription payment
updates или audit/reconciliation через `/startransactions`.

Pinned `aiogram==3.3.0` не имеет typed wrapper для этого метода, поэтому
реализация идет через изолированный raw Bot API helper
`bot/services/edit_user_star_subscription.py`. Helper валидирует положительный
integer `user_id`, непустой charge id и boolean `is_canceled`, POST'ит JSON
payload `{"user_id": ..., "telegram_payment_charge_id": ..., "is_canceled":
...}` на endpoint `editUserStarSubscription` через `httpx`, берет URL через
`bot.session.api.api_url(...)` для поддержки local Bot API server и поднимает
validation errors, транспортные ошибки, невалидный JSON, Telegram `ok: false`
и неожиданный result как `EditUserStarSubscriptionError`.

Сценарий намеренно отделен от `/sendinvoice`, `/createinvoicelink`,
`/startransactions`, shipping/pre-checkout callbacks и `free-claude-code`:
оператор сначала сверяет billing audit log и charge id, затем запускает
subscription edit отдельной командой с явным `confirm`. Для самой admin-команды
не нужны специальные update types или chat administrator rights; production
billing, который формирует данные для subscription edit, должен сохранять
subscription payment и renewal audit data отдельно.

Команда доступна только chat id из `TELEGRAM_ADMIN_CHAT_IDS` и не делает
fallback на `TELEGRAM_ALLOWED_CHAT_IDS`; при пустом admin allowlist команда
отключена. Handler хранит in-process idempotency set по тройке `(user_id,
telegram_payment_charge_id, is_canceled)`, поэтому повторная подтвержденная
команда в рамках того же процесса не вызывает Telegram второй раз и возвращает
audit-сообщение о ранее записанном subscription edit.

Security/privacy impact: команда меняет финансовое состояние Stars
subscription renewal, поэтому она не вызывается автоматически и требует
отдельный admin allowlist. Structured logs содержат user id, charge id, target
state и форму ошибки для audit, но не полный billing payload. Rollback
операционный: выполнить команду с противоположным target state, если Telegram
это разрешает, убрать chat id из `TELEGRAM_ADMIN_CHAT_IDS`, удалить handler или
отключить billing module вокруг helper.
